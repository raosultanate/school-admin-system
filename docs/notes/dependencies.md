...
|---|---|---|
| `spring-boot-starter-webmvc` | Embedded Tomcat + Spring MVC — lets us write `@RestController`s and run a web server. Called `webmvc` (not the older `web`) in Boot 4 to distinguish it from the reactive stack, `spring-boot-starter-webflux`, which we're *not* using. | Module 3 (REST APIs) |
| `spring-boot-starter-data-jpa` | Spring Data JPA + Hibernate — ORM that maps Java objects to database rows, and lets us write repository interfaces instead of SQL. | Module 2 (Persistence) |
| `spring-boot-starter-validation` | Jakarta Bean Validation (Hibernate Validator) — annotations like `@NotNull`, `@Size` to validate incoming data declaratively. | Module 4 (Validation) |
| `h2` (`com.h2database`) | The actual H2 database engine — a real SQL database that runs in-memory or in a file, no separate server to install. Good for learning/dev; we'll swap to Postgres later (Module 9). `runtime` scope: our code never imports H2 classes directly, Spring Data JPA talks to it through JDBC. | Module 2 |
| `spring-boot-h2console` | The H2 web console (a browser UI at `/h2-console` to poke at the dev database directly). Split out as its own starter in Boot 4 — in older Spring Boot versions this was bundled differently. | Module 2 |
...
| `lombok` | Generates boilerplate (getters/setters/constructors) from annotations like `@Getter`, `@Data` at compile time, so you don't hand-write it. **Not used yet** — `Person`/`Student`/`Teacher` (from the `java-refresher` project) were written by hand on purpose, so the constructor/getter mechanics were visible while learning them. We'll reach for Lombok once that's second nature (likely Module 2, on the JPA entities, where the boilerplate gets real). |  Module 2+ |
| `spring-boot-starter-actuator` | **Not requested** — Boot 4's Initializr appears to include it by default now. Exposes production monitoring endpoints (`/actuator/health`, `/actuator/metrics`, etc.). Harmless to leave in; we'll deliberately use it around Module 9 when we talk about running this in something closer to production. |  Module 9 |
...
We'll use these in Module 7 (Testing).
