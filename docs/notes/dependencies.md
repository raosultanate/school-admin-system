...
|---|---|---|
| `spring-boot-starter-webmvc` | Embedded Tomcat + Spring MVC — lets us write `@RestController`s and run a web server. Called `webmvc` (not the older `web`) in Boot 4 to distinguish it from the reactive stack, `spring-boot-starter-webflux`, which we're *not* using. | Module 5 (REST APIs) |
| `spring-boot-starter-data-jpa` | Spring Data JPA + Hibernate — ORM that maps Java objects to database rows, and lets us write repository interfaces instead of SQL. | Module 4 (Persistence) |
| `spring-boot-starter-validation` | Jakarta Bean Validation (Hibernate Validator) — annotations like `@NotNull`, `@Size` to validate incoming data declaratively. | Module 6 (Validation) |
| `h2` (`com.h2database`) | The actual H2 database engine — a real SQL database that runs in-memory or in a file, no separate server to install. Good for learning/dev; we'll swap to Postgres later (Module 11). `runtime` scope: our code never imports H2 classes directly, Spring Data JPA talks to it through JDBC. | Module 4 |
| `spring-boot-h2console` | The H2 web console (a browser UI at `/h2-console` to poke at the dev database directly). Split out as its own starter in Boot 4 — in older Spring Boot versions this was bundled differently. | Module 4 |
...
We'll use these in Module 9 (Testing).
